# UPDATE: [12/01/2026] See reworked_eurlex_query.R for the finalised code to pull EurLex metadata. 

# Author: Katelyn Nutley
# Title: Accessing EU Pre-Legislative Documents Metadata via EurLex R Package 
# Date: 16-07-2025

# Author note; this was previously called first_scrape_partial_eurlex.R, but was changed because it only produced metadata 
# and required a Python scraper later. 
...
install.packages(c("eurlex", "wordcloud", "purrr", "ggiraph", "rvest", "testthat")) # suggested dependencies in documentation
# only call the function if (like me), you don't already have these packages installed
packages <- c("dplyr", "eurlex", "ggiraph", "ggplot2", "httr", "knitr", "purrr", "rmarkdown", "rvest", "testthat", "tidyr", "wordcloud")
